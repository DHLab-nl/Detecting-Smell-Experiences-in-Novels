"""
Example
    $ python3 collect_by_pattern.py parsed_extracts.json
"""
...
def convert_patterns(patterns):
    ...
    Args:
        patterns (list): see PATTERNS.py
        chuncks (dict): see CHUNKS.py
    Returns:
...
def convert_chunks(pattern):
    ...
    Returns: 
        'simple' pattern with expanded chunks
    """
    blocks = []
        ...
        for extract in tqdm(extracts):
            ...
            for simple_pattern, pattern in converted_patterns:
                ...
                if mo:
                    ...
                    # assemble new lexicon terms
                    if any(mo.groups()):
                        if simple_pattern in lexicon.keys():
                            lexicon[simple_pattern] += [mo.groups()]
                        else: 
                            lexicon[simple_pattern] = [mo.groups()]
                    break
