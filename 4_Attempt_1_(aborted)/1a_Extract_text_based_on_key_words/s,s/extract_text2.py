...
def convert_tagging_format(doc):
    ...
    Return:
        a list of sentences, each sentence a list of (dep, pos, token_pos) tuples
    """
    ...
    for sent_index, sent in enumerate(doc.sents):
        ...
        for token in sent:
            if token.pos_ != "SPACE":
                output[-1].append(
                    (token.dep_, token.pos_, token.text + "_" + token.pos_)
                )
...
def extract_matches(seeds, sentences):
    ...
    for sentence in sentences:
        overlap = list(set(seeds).intersection(set([token[2] for token in sentence])))
        if any(overlap):
