...
def convert_patterns(patterns, chunks=chunks):
    ...
    # iterate through (abstracted) patterns
    for abstracted_pattern in patterns:

        # replace chunks with abstracted patterns
        expanded_pattern = expand_chunks(abstracted_pattern, chunks)

        # store for converted pattern
        python_pattern = []

        # convert each token to a python pattern
        tokens = __get_tokens(expanded_pattern) 

        # bracket convertion abstracted to python pattern
        conversion = {
            # groups
            "{": "(?:",
            "}*": ")*",
        ...
        # iterate over blocks and append converted block to python pattern
        for token_index, token in enumerate(tokens):

            # reset for handling block *, +
            zero_or_more, one_or_more = False, False

            # convert/ append brackets
            if token in conversion.keys():
                python_pattern.append(conversion(token))
            # convert/ append other blocks
            else:
                ...
                # if any of dep, text, pos is undefined...redefine \S+
                if dep == "":
                    dep = "\S+"
                if text == "":
                    text = "\S+"
        ...
        pattern_as_string = ""
        last_term = None
        # append each item to python_pattern, accounting for correct spaces
...
def expand_chunks(abstracted_pattern, chunks):
    """Replace <chunk> entries in abstracted_pattern
    ...
    """
    # iterative through tokens in abtracted pattern
    tokens = __get_tokens(abstracted_pattern)
...
def __get_tokens(abstracted_pattern):
    """Return an abstracted pattern, as a list of tokens.
       chunks are converted to abtracted elements
    """

    blocks = []
    for block in re.split(r"\s+|(}\*|}\+|}|\]|{|\[)", abstracted_pattern):
